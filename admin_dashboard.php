...
function adminReportsTable(PDO $pdo): ?string
{
    ...
    if (adminTableExists($pdo, 'segnalazioni')) {
        return 'segnalazioni';
    }
    return null;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        if ($action === 'resolve_report') {
            ...
            $reportsTable = adminReportsTable($pdo);
            if ($reportId <= 0 || !$reportsTable) {
                throw new RuntimeException('Segnalazione non valida.');
            }

            $resolvedStatus = $reportsTable === 'segnalazioni' ? 'chiusa' : 'risolta';
            $stmt = $pdo->prepare("UPDATE `$reportsTable` SET stato = ? WHERE id = ?");
            $stmt->execute([$resolvedStatus, $reportId]);
...
} elseif ($reportsTable === 'segnalazioni') {
    $reportsSql = "
        SELECT
            s.id,
            s.segnalante_id AS segnalatore_id,
            s.segnalato_id,
            rr.offer_id AS offerta_id,
            s.tipo AS motivo,
            s.descrizione,
            s.stato,
            s.created_at,
            ...
            reported.email AS reported_email
        FROM segnalazioni s
        LEFT JOIN ride_requests rr ON rr.id = s.request_id
        LEFT JOIN users reporter ON reporter.id = s.segnalante_id
        LEFT JOIN users reported ON reported.id = s.segnalato_id
        ORDER BY
            CASE WHEN s.stato = 'aperta' THEN 0 ELSE 1 END,
            s.created_at DESC,
            s.id DESC
    ";
    $reports = $pdo->query($reportsSql)->fetchAll();
...
$openReports = $reportsTable
    ? (int)$pdo->query("SELECT COUNT(*) FROM `$reportsTable` WHERE stato = 'aperta'")->fetchColumn()
    : 0;
    ...
    <div class="container">
        ...
        <section class="admin-panel">
            ...
            <div id="reports-content" class="admin-content <?= $activeTab === 'reports' ? 'active' : '' ?>">
                ...
                <?php if (!$reportsTable): ?>
                    <div class="admin-empty"><i class="fas fa-database"></i>La tabella segnalazioni non e' ancora presente nel database.</div>
                <?php elseif (!$reports): ?>
